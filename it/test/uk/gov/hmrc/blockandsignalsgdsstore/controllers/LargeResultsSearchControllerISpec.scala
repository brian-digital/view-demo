...
class LargeResultsSearchControllerISpec extends BaseISpec with ScalaFutures with IntegrationPatience with Generators with ScalaCheckDrivenPropertyChecks with Logging {
  ...
  implicit val actorSystem: ActorSystem = ActorSystem("LargeResultsSearchControllerISpec")
