...
class SearchControllerISpec extends BaseISpec with ScalaFutures with IntegrationPatience with Generators with ScalaCheckDrivenPropertyChecks {
  ...
  "POST /search/results" should {
    ...
    "return search results for accountIntervention: for accountIntervention search only InitiatingEntity-analyst will return" in {
      ...
      val isAnalystDocument = eventToDocument(eventIsAnalystInitiatingEntity)

      val eventIsNotAnalystInitiatingEntity: Event = interventionEventGen(isAnalyst = false).sample.get
      val isNotAnalystDocument = eventToDocument(eventIsNotAnalystInitiatingEntity)

      val eventDocument = eventToDocument(eventIsAnalystInitiatingEntity)
    ...
    "return search results for the the other events" in {
      forAll(eventsConcernEventBetweenGen(defaultFrom.toInstant, defaultTo.toInstant)) { events =>
        val documents = events.map(eventToDocument)
